...
# if not working, pascal_label_map_dict change!
...
def main(_):
  ...
  for year in years:
    example_class = list(label_map_dict.keys())[0] # if not working, [0] to [1] (this value was default)
    ...
    for idx, example in enumerate(examples_list):
      ...
      with tf.io.gfile.GFile(path, 'r') as fid:
        xml_str = fid.read()
      xml = etree.fromstring(xml_str.encode('utf-8'))  # UTF-8로 인코딩
